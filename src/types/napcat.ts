...
export interface NapCatPluginContext {
  ...
  // Current NapCat releases provide the absolute config.json file path here.
  // Older plugin hosts may provide the containing directory instead.
  configPath: string;
...
export interface PluginModule {
  ...
  plugin_onmessage?: (ctx: NapCatPluginContext, event: OB11MessageInput) => void | Promise<void>;
  plugin_onevent?: (ctx: NapCatPluginContext, event: OB11EventInput) => void | Promise<void>;
...
export interface OB11NoticeEvent extends OB11BaseEvent {
  ...
  group_id?: OneBotId;
  user_id?: OneBotId;
  operator_id?: OneBotId;
  sub_type?: string;
}

// Provider boundary shapes. NapCat/SnowLuma may emit safe numbers or exact
// decimal strings; handlers normalize these before shared application code.
export interface OB11BaseEventInput extends Omit<OB11BaseEvent, 'self_id'> {
  self_id: OneBotIdInput;
}

export interface OB11MessageInput extends Omit<OB11Message, 'self_id' | 'message_id' | 'user_id' | 'group_id' | 'message' | 'sender'> {
  self_id: OneBotIdInput;
  message_id: OneBotMessageIdInput;
  user_id: OneBotIdInput;
  group_id?: OneBotIdInput;
  message: OB11MessageSegment[] | string;
  sender: Omit<OB11Sender, 'user_id'> & { user_id: OneBotIdInput };
}

export interface OB11RequestEventInput extends Omit<OB11RequestEvent, 'self_id' | 'group_id' | 'user_id'> {
  self_id: OneBotIdInput;
  group_id?: OneBotIdInput;
  user_id: OneBotIdInput;
}

export interface OB11NoticeEventInput extends Omit<OB11NoticeEvent, 'self_id' | 'group_id' | 'user_id' | 'operator_id'> {
  self_id: OneBotIdInput;
  group_id?: OneBotIdInput;
  user_id?: OneBotIdInput;
  operator_id?: OneBotIdInput;
}

export type OB11EventInput = OB11RequestEventInput | OB11NoticeEventInput;
